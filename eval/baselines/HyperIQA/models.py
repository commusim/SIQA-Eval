...
class HyperNet(nn.Module):
    ...
    def __init__(self, lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size):
        ...
        self.fc5w_fc = nn.Linear(self.hyperInChn, self.f4 * 2)  # 权重 shape: [B, f4*2]
        self.fc5b_fc = nn.Linear(self.hyperInChn, 2)           # 偏置 shape: [B, 2]
        # initialize
    ...
    def forward(self, img):
        ...
        # fc5w: [B, f4*2] -> view as [B, 2, f4, 1, 1]
        target_fc5w = self.fc5w_fc(self.pool(hyper_in_feat).squeeze()).view(-1, 2, self.f4, 1, 1)
        # fc5b: [B, 2] -> view as [B, 2]
        target_fc5b = self.fc5b_fc(self.pool(hyper_in_feat).squeeze()).view(-1, 2)
        out = {}
...
class TargetNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # q = self.l4(q).squeeze()
        # return q
        q = self.l4(q)  # shape: [B, 2, 1, 1]
        return q.squeeze(-1).squeeze(-1)  # → [B, 2]
...
def resnet50_backbone(lda_out_channels, in_chn, pretrained=False, **kwargs):
    ...
    Args:
        lda_out_channels (int): Number of output channels for LDA layer.
    ...
    if pretrained:
        # 指定你本地的 .pth 文件路径
        local_path = '/mnt/shared-storage-user/liwenzhe/SIQA-Baseline/resnet50-19c8e357.pth'  # 👈 改成你的实际路径
        save_model = torch.load(local_path, map_location='cpu', weights_only=False)  # 加载到 CPU
        ...
        # 如果不使用预训练模型，则初始化权重
        model.apply(weights_init_xavier)  # 假设你有一个函数 weights_init_xavier 来初始化权重
