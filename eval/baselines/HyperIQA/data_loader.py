...
class DataLoader(object):
    ...
    def __init__(self, dataset, path, img_indx, patch_size, patch_num, batch_size=1, istrain=True):
        ...
        if (dataset == 'live') | (dataset == 'csiq') | (dataset == 'tid2013') | (dataset == 'livec'):
            ...
            if istrain:
                ...
                transforms = torchvision.transforms.Compose([
                    ...
                    torchvision.transforms.Normalize(mean=(0.485, 0.456, 0.406),
                                                     std=(0.229, 0.224, 0.225))])
        # 在 transforms 定义部分，为 SIQA 单独处理
        elif dataset == "SIQA":
            if istrain:
                transforms = torchvision.transforms.Compose([
                    torchvision.transforms.RandomHorizontalFlip(),
                    torchvision.transforms.Resize((512, 512)),  # 可选：统一尺寸
                    torchvision.transforms.RandomCrop(size=patch_size),
                    torchvision.transforms.ToTensor(),
                    torchvision.transforms.Normalize(mean=(0.485, 0.456, 0.406),
                                                std=(0.229, 0.224, 0.225))
                ...
                transforms = torchvision.transforms.Compose([
                    torchvision.transforms.Resize((512, 512)),
                    torchvision.transforms.CenterCrop(size=patch_size),  # 测试用 CenterCrop 更稳定
                    torchvision.transforms.ToTensor(),
                    torchvision.transforms.Normalize(mean=(0.485, 0.456, 0.406),
                                                std=(0.229, 0.224, 0.225))
                ])
        if dataset == 'live':
            ...
            if self.istrain:
                self.data = folders.SIQAFolder(root=path,
                                               json_file_path="/mnt/shared-storage-user/liwenzhe/SIQA-Baseline/train_ratings.json", 
                                               transform=transforms)
            else:
                self.data = folders.SIQAFolder(root=path,
                                               json_file_path="/mnt/shared-storage-user/liwenzhe/SIQA-Baseline/bench_ratings.json", 
                                               transform=transforms)
