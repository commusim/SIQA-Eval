...
class SIQAFolder(data.Dataset):    
    def __init__(self, root, json_file_path, transform=None):
        """
        初始化数据集

        参数:
            root (str): 图像文件所在的根目录
            json_file_path (str): 包含图像路径和评分信息的JSON文件路径
        ...
        for item in self.data:
            image_path = os.path.join(root, item['image_path'])
            perception_rating = item.get('subjective_rating', None)
            knowledge_rating = item.get('objective_rating', None)
            if perception_rating is not None and knowledge_rating is not None:
                self.samples.append((image_path, perception_rating, knowledge_rating))
    ...
    def __getitem__(self, index):
        ...
        返回:
            tuple: (sample, perception_target, knowledge_target) 其中 target 是目标等级
        """
        path, perception_target, knowledge_target = self.samples[index]
        sample = pil_loader(path)
        
        if self.transform is not None:
            sample = self.transform(sample)

        return sample, [perception_target, knowledge_target]
